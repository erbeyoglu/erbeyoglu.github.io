...
(() => {
  ...
  function questionHTML(q, answerButtons, practice = false, bank = MODELING) {
    ...
    const label=practice ? `${q.challenge ? 'Extra · ' : q.stretch ? 'Stretch · ' : ''}Question ${number} of ${bank.forWeek(q.week).length}` : `${q.optional ? 'Reserve' : 'Core'} checkpoint`;
  ...
  const practiceTopics = {
    ...
    week13:'DTMC: long-run and first passage', week14:'Continuous-time Markov chains'
  };
  const practiceCache=new Map();
  let practiceStorageOK=true;
  function practiceRecord(q) {
  ...
  function renderPractice() {
    ...
    const nextLabel=!isClassPreview && index===2?'Try more questions →':'Next question →';
    ...
    const updateProgress=()=>{
      ...
      if(isClassPreview)$('practice-progress').textContent=reviewedCount(week)+' of '+all.length+' explanations reviewed · '+storage;
        ...
        $('practice-progress').textContent='Starting questions '+startingReviewed+' of '+starting.length+' · Extra questions '+challengeReviewed+' of '+challenges.length+' reviewed · '+storage;
      }
