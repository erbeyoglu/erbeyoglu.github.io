(() => {
  ...
  // Anonymous progress counts for the instructor (assets/usage.js): which step
  // actions happened and how far a student got, never what they wrote.
  const track=(action,max)=>document.dispatchEvent(new CustomEvent('ie301:usage',{detail:{event:'guided:'+activity.id+':'+action,max}}));
  let assembledCounted=false;
  ...
  function render() {
    ...
    app.innerHTML=`<section class="widget"><div class="learning-eyebrow">Week ${Number(activity.week.slice(4))} · ${escape(activity.family)}</div><h2>${escape(activity.title)}</h2><p class="learning-context">${escape(activity.intro)}</p>
      ...
      <p id="storage-note" class="learning-muted">${storageWorks?'Progress and notes are saved in this browser. Nothing you write is sent anywhere; the site only counts, anonymously, how far students get.':'Browser storage is unavailable. Download your work before leaving.'}</p>
    ...
    $('guided-download').onclick=()=>{track('download');download();};
    if(atEnd && !assembledCounted){assembledCounted=true;track('assembled',done);}
  ...
  function renderStep() {
    ...
    $('guided-hint').onclick=()=>{s.hint=!s.hint;if(s.hint)track('hint');persist();renderStep();};
    ...
    $('guided-check').onclick=()=>{
      ...
      track('check');if(s.correct)track('correct',count());
      persist();render();
    };
    $('guided-reveal').onclick=()=>{
      if(!s.attempts && !s.hint){s.hint=true;track('hint');persist();renderStep();$('guided-feedback').hidden=false;$('guided-feedback').textContent='Try the hint, then compare with the worked step when you are ready.';return;}
      s.revealed=true;track('worked',count());persist();render();
    };
  ...
  function renderEnd() {
    ...
    $('twist-compare').onclick=()=>{state.twistRevealed=true;track('twist');persist();$('twist-answer').hidden=false;};
  }
  function download() {
