...
window.PRECLASS = (() => {
  ...
  add("01",
    "straight-path",
    "Can every straight path stay inside?",
    "A square courtyard includes its boundary but has a circular pond in its center. Walking through the pond is forbidden. A designer claims that the straight segment between any two allowed positions stays entirely in the walking area.",
    "Which test settles the claim?",
    ["Choose two allowed positions on opposite sides of the pond whose joining segment crosses it", "Choose two allowed positions along the same outer edge", "Check that every allowed position can be reached by some curved path"],
    0,
    ["This one pair disproves the claim: part of its straight segment is forbidden.", "One segment that stays inside cannot prove a claim about every pair.", "A route around the pond may exist even when the straight segment is forbidden."],
    "Convex sets keep the entire straight segment between every pair of their points; one failing pair is enough to disprove convexity.");
  add("01",
    "chord-comparison",
    "Compare the middle with the straight line",
    "A reward curve has values 0 at effort 0, 8 at effort 2, and 12 at effort 4. Compare the middle observation with the straight line joining the two endpoint observations.",
    "Where is the middle observation relative to that line?",
    ["Below it, because 8 is less than 12", "On it, because effort 2 is halfway", "Above it: the line has height 6 there"],
    2,
    ["Compare with the line at effort 2, not with its right endpoint.", "A halfway input need not produce the halfway output.", "The endpoint average is (0 + 12)/2 = 6, while the actual reward is 8."],
    "A concave curve lies above its endpoint chords. This observation fits that direction but three observations cannot establish concavity everywhere.");
  add("01",
    "interior-best",
    "Must the best setting be an endpoint?",
    "A machine allows any real setting x between 0 and 8. Its score is 20 minus the square of (x − 3). The operator plans to compare only settings 0 and 8.",
    "What does that plan miss?",
    ["Nothing: one endpoint must maximize any score on an interval", "Setting 3 scores 20, and no other setting scores more", "The score has no maximum because infinitely many settings are allowed"],
    1,
    ["An endpoint rule does not hold for a general nonlinear score.", "A square is nonnegative and is zero at x = 3, so the score is at most 20 and reaches it inside the interval.", "Infinitely many choices do not prevent a maximum from being attained."],
    "A nonlinear objective can attain its optimum inside a convex feasible region; boundary checks alone are insufficient.",
    true);
  ...
  add("03",
    ...
    "Compare equally sized moves",
    "Near a current setting, a score change is approximated by 3Δx + 4Δy. Compare three tiny moves with the same length: A changes (x,y) by (ε,0), B by (0,ε), and C by (0.6ε,0.8ε), where ε is small and positive.",
    "Which move has the largest predicted score gain?",
    ["Move A", "Move B", "Move C"],
    2,
    ["A predicts a gain of 3ε.", "B predicts a gain of 4ε.", "C predicts 3×0.6ε + 4×0.8ε = 5ε, greater than either axis move."],
    "The gradient combines partial changes into a direction of improvement; comparing equal-length moves separates direction from step size.",
    true);
  ...
  add("04",
    "stay-on-equality",
    "Improve while keeping the equality",
    "Two allocations satisfy x + y = 10 exactly. A proposed improvement increases x by 1. Both current allocations exceed 1, so a one-unit decrease remains nonnegative.",
    "Which simultaneous change keeps the proposal feasible?",
    ["Leave y unchanged", "Decrease y by 1", "Increase y by 1"],
    1,
    ["The total would become 11, breaking the equality.", "The changes +1 and −1 cancel, preserving the required total of 10.", "The total would become 12."],
    "Equality-constrained optimization must improve the objective along feasible changes; variables cannot always move independently.");
  ...
  add("05",
    "square-increments",
    "What rule explains the accumulated increments?",
    "At setting 0, added drain is 0. Raising the setting by one notch adds 1, then 3, then 5, then 7 percentage points of drain. Let n denote the number of notches, from 0 through 4.",
    "Which rule matches the total added drain at every recorded setting?",
    ["2n", "n²", "n³"],
    1,
    ["At n = 3 this predicts 6, but the accumulated drain is 1+3+5 = 9.", "The totals are 0, 1, 4, 9, 16: the squares of the recorded settings.", "At n = 2 this predicts 8 instead of 1+3 = 4."],
    "A quadratic term has changing marginal effects. Matching these observations identifies a candidate rule, not proof of its behavior at unobserved settings.");
  ...
  addChallenge("01",
    "and-or-regions",
    "What happens when safe regions are combined?",
    "Two regions each have this property: every straight segment joining two points in the region stays inside it. Their overlap is nonempty. Compare the region allowed by BOTH rules with the region allowed by EITHER rule.",
    "Which statement must hold?",
    ["Both the overlap and the either-rule region always keep the property", "The overlap keeps the property, but the either-rule region may lose it", "Neither region can keep the property"],
    1,
    ["Two overlapping disks can have a non-convex union; a segment can leave both disks.", "Endpoints in the overlap belong to each region, so the full segment belongs to each too. For the union, the endpoints may belong to different regions.", "The segment argument guarantees the property for the overlap."],
    "Intersections of convex sets are convex. Unions need not be, even when the sets overlap.");
  addChallenge("01",
    "both-shape-rules",
    "Can a line satisfy both rules?",
    "Rule L requires each value between two inputs to be at most the corresponding weighted average of the endpoint values. Rule U requires it to be at least that same average. A tariff on all nonnegative quantities is 5 plus 3 times the quantity.",
    "Which rule does this tariff satisfy for every pair of inputs and every blend between them?",
    ["Both L and U, because the values are exactly equal to their endpoint blends", "Only L, because the tariff increases", "Neither, because the graph is not curved"],
    0,
    ["For a blend of x and y, distributing 5 + 3[λx + (1−λ)y] gives exactly λ(5+3x) + (1−λ)(5+3y). Equality satisfies both inequalities.", "Increasing or decreasing describes slope, not the direction relative to a chord.", "The non-strict rules permit equality everywhere."],
    "An affine function is both convex and concave, but neither strictly convex nor strictly concave on an interval with distinct points.");
  addChallenge("01",
    "hidden-direction",
    "Can two bowl-shaped slices mislead?",
    "A surface has height h(x,y) = x² + y² − 4xy. Along y = 0 it is x²; along x = 0 it is y², so both axis slices are upward bowls. Someone concludes the entire surface is bowl-shaped.",
    "What does examining the diagonal x = y = t reveal?",
    ["Another upward bowl: h(t,t) = 2t²", "A flat line: h(t,t) = 0", "A downward bowl: h(t,t) = −2t², contradicting the conclusion"],
    2,
    ["This omits the −4xy interaction.", "The two square terms sum to 2t², while the interaction is −4t².", "Substitution gives t² + t² − 4t² = −2t². Two reassuring axis slices missed a direction of opposite curvature."],
    "For multivariable convexity, interactions and all directions matter; this motivates checking the whole Hessian rather than only its diagonal.");

  addChallenge("02",
    "local-quadratic-step",
    "Let a local curve propose the next trial",
    "At the current setting x = 3, a local approximation predicts the score after a change t as 10 + 6t − t² = 19 − (t − 3)². It is only an approximation near the current setting, not a formula known to hold everywhere.",
    "What can the approximation justify?",
    ["The next trial is x = 3 because the best change is zero", "Try x = 6, then check the true objective; the approximation does not guarantee a global optimum", "x = 6 is guaranteed to be the true global optimum"],
    1,
    ["The predicted score is largest at change t = 3, not t = 0.", "The approximation peaks at t = 3, so x + t = 6 is its proposed next trial. The real objective still needs checking.", "A local approximation can be inaccurate away from the current setting and does not certify a global solution."],
    "Newton’s method maximizes a local quadratic approximation to propose a step; its local model is not a global guarantee.");
  ...
  addChallenge("02",
    "complete-candidates",
    "Keep every kind of candidate",
    "For a continuous score on a closed interval, a maximum can occur at an endpoint, an interior point with zero slope, or an interior point where the slope does not exist. On [0,8], the only zero-slope point is 2 and the only undefined-slope point is 5.",
    "Which complete candidate set should be compared?",
    ["0, 2, 5 and 8", "2 only", "2 and 5 only"],
    0,
    ["This includes both endpoints and both special interior points.", "A zero slope is only one way a maximum can occur.", "These are all the interior candidates, but either endpoint could still score more."],
    "For general one-variable NLP, finding stationary points is only part of the search; endpoints and nondifferentiable points also matter.");
  ...
  addChallenge("03",
    "flat-but-saddle",
    "Flat does not settle the classification",
    "For h(x,y) = x⁴ − y⁴, both first partial derivatives are zero at (0,0), where the height is 0. You can examine points arbitrarily close to the origin by choosing a positive ε as small as needed.",
    "What do (ε,0) and (0,ε) show?",
    ["The origin is a local minimum because all first derivatives vanish", "There are both higher and lower points arbitrarily close, so the origin is neither a local minimum nor a local maximum", "The origin is a local maximum because one nearby value is negative"],
    1,
    ["At (0,ε) the height is −ε⁴, below the origin.", "The two heights are ε⁴ and −ε⁴ for every ε > 0. Vanishing first derivatives have not identified a peak or a bowl.", "At (ε,0) the height is positive, so nearby points can also be higher."],
    "Stationarity gives candidates, not a classification. Here even the Hessian at the origin is zero: the second-order test is inconclusive and nearby values resolve the saddle.");
  addChallenge("03",
    "direction-and-distance",
    "Choose a step, then update both coordinates",
    "A search starts at (1,1) and follows direction (2,−1). With step parameter t ≥ 0 its trial point is (1+2t,1−t). Along this line, the exact score is 18 − 2(t−2)².",
    "Which point does the best step along this line reach?",
    ["(5,−1)", "(2,−1)", "(3,0)"],
    0,
    ["The score peaks at t = 2. Substituting gives (1+4,1−2) = (5,−1).", "This is the direction vector itself, not the starting point plus the chosen step.", "This uses t = 1, whose score is 16 instead of the maximum 18."],
    "Gradient search first chooses a direction, then solves a one-variable line search for the step parameter before updating the full point.");
  ...
  addChallenge("05",
    ...
    "Balance the penalty against the linear reward",
    "Two nonnegative real allocations satisfy x+y=6. Their score is 8x−(x−y)²: x earns a linear reward, while imbalance incurs a squared penalty. After using y=6−x, the score can be written as 28−4(x−4)².",
    "Which allocation maximizes the score over the whole feasible interval?",
    ["x=3, y=3: eliminate all imbalance", "x=6, y=0: maximize the linear reward alone", "x=4, y=2: accept some imbalance for more reward"],
    2,
    ["The score is 24: avoiding every penalty sacrifices more reward than necessary.", "The score is 48−36 = 12; the imbalance penalty is too large.", "The completed square is never negative, so the score is at most 28 and reaches 28 at x=4, y=2."],
    "A QP combines linear rewards with coupled quadratic effects under linear constraints. Optimize the complete objective rather than any one term in isolation.");
  ...
  addChallenge("08",
    "setup-storage",
    "Trade setups for storage",
    "A bakery needs two trays on Monday, two on Tuesday and two on Wednesday. Starting the ovens on a day costs 6 credits. Storing one tray overnight costs 1 credit. Production cost per tray is constant; capacity is unlimited and trays remain usable throughout the three days.",
    "Which listed schedule has the lowest setup-plus-storage cost?",
    ["Make two trays each day", "Make all six on Monday", "Make four on Monday and two on Wednesday"],
    1,
    ["Three starts cost 18 credits, with no storage.", "One start costs 6. Two trays wait one night and two wait two nights: storage is 2 + 4 = 6, for a total of 12.", "Two starts cost 12; two trays wait one night, adding 2, for a total of 14."],
    "The lot-sizing problem trades fewer setups against the cost of carrying inventory across periods.");
  addChallenge("08",
    "all-sites-pass",
    "Allocate boosts when every site must pass",
    "A project succeeds only if three independent sites all pass; multiply their pass chances to obtain project success. Without a kit their chances are A: 0.90, B: 0.60, C: 0.40. A kit changes its chosen site to A: 0.95, B: 0.80, or C: 0.70. Two kits must go to different sites.",
    "Which two sites should receive the kits?",
    ["A and B", "A and C", "B and C"],
    2,
    ["This gives 0.95 × 0.80 × 0.40 = 0.304.", "This gives 0.95 × 0.60 × 0.70 = 0.399.", "This gives 0.90 × 0.80 × 0.70 = 0.504, the largest project-success chance."],
    "In nonadditive resource allocation, compare complete allocations using the required product of outcomes.");
  ...
  addChallenge("09",
    ...
    "Pay now and account for what follows",
    "Choice A costs 2 credits now, then 2 more with probability 3/4 or 10 more with probability 1/4. Choice B costs 4 now, then 1 or 5 more with equal probability. Choose before learning the future cost.",
    "Which choice has the lower average total cost over repeated trials?",
    ["Choice A", "Choice B", "They have the same average total cost"],
    0,
    ["A costs 2 + (3/4)×2 + (1/4)×10 = 6 on average.", "B costs 4 + (1/2)×1 + (1/2)×5 = 7 on average.", "The average totals are 6 and 7, a difference of 1."],
    "A probabilistic DP compares each current action together with the probability-weighted future costs it creates.");
  addChallenge("09",
    "stop-or-wait",
    "Leave now or keep waiting",
    "Two minutes remain before a desk closes. Leaving gives 0 points. Waiting for a minute costs 1 point; at its end you are served with probability 1/2 and earn 6 points, ending the game. If not served and one minute remains, you may choose again with the same conditional chances. Closing unserved gives 0 further points.",
    "Which plan has the highest average net points?",
    ["Leave immediately", "Wait now and, if needed, wait through the final minute", "Wait once, then leave if not served"],
    1,
    ["Leaving gives 0.", "With one minute left, waiting is worth −1 + (1/2)×6 = 2. From the start the value is −1 + (1/2)×6 + (1/2)×2 = 3.", "One attempt is worth −1 + (1/2)×6 = 2, below the two-attempt plan."],
    "Work backward in a stopping problem: compare stopping now with the current cost, possible reward and best continuation.");
  ...
  addChallenge('10','two-days','Reach tomorrow through today',
    ...
    ['28%','30%','2%'],0,
    ['Failure today contributes 20%; surviving today and then failing contributes 80% × 10% = 8%, for 28% in total.','Adding 20% and 10% ignores that tomorrow’s failure branch is reached only after survival today.','This multiplies 0.20 by 0.10, but tomorrow’s failure branch is reached after survival today. That branch uses 0.80 × 0.10, and failure today must also be counted.'],
  ...
  addChallenge("13",
    "eventual-win",
    "A replay is not a final outcome",
    "In each round of a game still in progress, you win with probability 1/4, lose with probability 1/2, or replay with probability 1/4. After a replay the same chances apply. Winning and losing end the game permanently.",
    "What is the probability of eventually winning, counting any number of replays?",
    ["1/3", "1/4", "1/2"],
    0,
    ["Let p be the eventual win chance. A win now contributes 1/4; a replay contributes (1/4)p, so p = 1/4 + p/4 and p = 1/3.", "This counts only wins on the first round, omitting wins after a replay.", "The final outcomes need not be equally likely; losing is twice as likely as winning at each deciding round."],
    "Absorption probabilities include all paths to a final outcome, including repeated returns to a temporary state.");
